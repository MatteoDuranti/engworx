...
using EngWorxCore.Library.Helpers.GenericWebGrid;
using EngWorxCore.Library;

namespace System.Web.Helpers
{
    public class GenericWebGrid
    {
        private List<GenericWebGridColumn> _columnList;
        private object _htmlAttributes;
        private string _totalRecordDesc;
        private string _totalRecordDescToolTip;
        private System.Web.Mvc.HtmlHelper _htmlHelper;
        //private bool _paged;
        private List<ImageExtensions.ImageLinkClass> _imageLinkListSx;
        private List<MvcHtmlString> _imageLinkList = null;
        private static int numbergridchk;
        ...
        private IList<GenericWebGridRow> _rows;

        private string _querystringGridname;
        ...
        public GenericWebGrid(System.Web.Mvc.HtmlHelper htmlHelper,
                            ISearchViewModel model,
                            bool Paged = true,
                            object htmlAttributes = null,
                            string ajaxUpdateContainerId = null,
                            string ajaxUpdateCallback = null,
                            string TotalRecordDesc = null,
                            List<ImageExtensions.ImageLinkClass> ImageLinkListSx = null,
                            string TotalRecordDescToolTip = null,
                            string QueryStringGridname =null)
            : this(ajaxUpdateContainerId: ajaxUpdateContainerId,
                ajaxUpdateCallback: ajaxUpdateCallback,
                rowsPerPage: model.DataParameters.pageSize,
                canPage: Paged)
        {
            bool autoSortAndPage = false;
            _htmlHelper = htmlHelper;
            _canPage = Paged;
            if (!Paged)
            {
                autoSortAndPage = true;
                model.DataParameters.totalRows = ((IList)model.Data).Count;
            }

            Bind((IEnumerable<object>)model.Data, rowCount: model.DataParameters.totalRows, autoSortAndPage: autoSortAndPage);
            // 03-07-2012 FIX: Aggiunto controllo se deve essere paginata
            if (Paged)
                Pager(WebGridPagerModes.All);
            // 03-07-2012 FIX: Aggiunto controllo ->> se è paginata allora viene impostato ip Page index
            if (model.DataParameters.totalRows > 0 && Paged)
            {
                PageIndex = model.DataParameters.pageNumber - 1;
            }
            SortColumn = model.DataParameters.sort;
            SortDirection = Utility.getSortDirection(model.DataParameters.sortDir);
            _htmlAttributes = htmlAttributes;
            _imageLinkListSx = ImageLinkListSx;
            if (!string.IsNullOrEmpty(TotalRecordDesc))
            {
                _totalRecordDesc = TotalRecordDesc;
            }
            _totalRecordDescToolTip = TotalRecordDescToolTip;
            _querystringGridname = QueryStringGridname;
        }

        private GenericWebGrid(
        ...
        public IList<GenericWebGridRow> Rows
        {
            get
            {
                EnsureDataBound();
        ...
        public GenericWebGridRow SelectedRow
        {
            get
            {
        ...
        public GenericWebGrid Bind(IEnumerable<dynamic> source, IEnumerable<string> columnNames = null, bool autoSortAndPage = true, int rowCount = -1)
        {
            if (_dataSourceBound)
            {
            ...
            if (!autoSortAndPage)
            {
                _dataSource = new GenericPreComputedGridDataSource(grid: this, values: source, totalRows: rowCount);
            }
            else
            {
                GenericWebGridDataSource dataSource = new GenericWebGridDataSource(grid: this, values: source, elementType: _elementType, canPage: _canPage, canSort: _canSort);
        ...
        public IHtmlString GetHtml()
        {
            ...
            {
                WebGridColumn col = _columnList.Find(m => m.ColumnName == SortColumn);
                string arrow = null;
                ...
                if ((col.Style == null || !col.Style.Contains(arrow)))
                {
                    col.Header += " " + arrow;
                }
            }
            ...
            if (_imageLinkListSx!=null || (!string.IsNullOrEmpty(_totalRecordDesc) & TotalRowCount > 0))
            {
            
                tableCaption.Append("<table cellpadding=\"0\" cellspacing=\"0\" style=\"width: 100%;border-style: none;margin: 0px 0px;\"><tr>");
                /** Pulsanti allineati a sinistra */
                tableCaption.Append("<td align=\"left\" style=\"border-style: none\">");
                tableCaption.Append("<div class='imagelistHeaderSX'>");
                if ((_imageLinkListSx != null))
                {
                    foreach (ImageExtensions.ImageLinkClass img in _imageLinkListSx)
                    {
                        if ((img.HasPermission))
                        {
                            tableCaption.Append(ImageExtensions.ImageLink(_htmlHelper, img.Action, img.Controller, img.RouteValue, img.ImageURL, img.AlternateText));
                        }
                    }
                }
                tableCaption.Append("</div>");

                tableCaption.Append("</td>");
                /** Totale righe */
                tableCaption.Append("<td align=\"right\" style=\"border-style: none\">");
                tableCaption.AppendFormat("<div class='divHeaderCount{1}' title='{0}'>", _totalRecordDescToolTip, (string.IsNullOrEmpty(_totalRecordDescToolTip) ? "" : " curHelp"));
                if (!string.IsNullOrEmpty(_totalRecordDesc) & TotalRowCount > 0)
                {
                    tableCaption.AppendFormat("Totale {0}: {1}", _totalRecordDesc, TotalRowCount.ToString());
                }
                tableCaption.Append("</div>");
                tableCaption.Append("</td>");
                /**********************************/
                tableCaption.Append("</table>");
            }

            StringBuilder builder = new StringBuilder();
            /* 31-08-2012 Commentato Old header DIV */
            //builder.AppendFormat("<div id='{0}Header'>", AjaxUpdateContainerId);
            //builder.AppendFormat("<div class='divHeaderCount{1}' title='{0}'>", _totalRecordDescToolTip, (string.IsNullOrEmpty(_totalRecordDescToolTip) ? "" : " curHelp"));
            //if (!string.IsNullOrEmpty(_totalRecordDesc) & TotalRowCount > 0)
            //{
            //    builder.AppendFormat("Totale {0}: <b>{1}</b>", _totalRecordDesc, TotalRowCount.ToString());
            //}
            //builder.Append("&nbsp;</div>");
            //builder.Append("<div class='imagelistHeaderSX'>");
            //if ((_imageLinkListSx != null))
            //{
            //    foreach (ImageExtensions.ImageLinkClass img in _imageLinkListSx)
            //    {
            //        if ((img.HasPermission))
            //        {
            //            builder.Append(ImageExtensions.ImageLink(_htmlHelper, img.Action, img.Controller, img.RouteValue, img.ImageURL, img.AlternateText));
            //        }
            //    }
            //}
            //builder.Append("&nbsp;</div>");
            ...
            // Controllo l'esistenza nelle colonne di notazioni per le checkbox
            foreach (GenericWebGridColumn c in _columnList)
            {
                if (c.GetType() == typeof(GenericWebGridColumnCheckBox))
                //if (!string.IsNullOrEmpty(c.Header) && c.Header.StartsWith("[]"))
                {
                    chkNumber++;
                    ...
                    nomecolonna = c.ColumnName;
                    numbergridchk++;
                    if (numbergridchk == int.MaxValue - 1)
                    {
                        numbergridchk = 0;
                    }
                    idallchk = "allchk_" + numbergridchk.ToString(); //chkNumber.ToString() + "_" + DateTime.Now.ToString("yyyyMMdd_hhmmssffff");        // Nome del componente header
                    //idchk = "chk" + chkNumber.ToString();
                    idchk = c.Header.Replace("[]", "").Trim();
                    keyname = "{[chk" + chkNumber.ToString() + "]}";
                    keyvalue = "<input type=\"checkbox\" id=\"" + idallchk + "\" name=\"" + idallchk + "\"  onclick=\"selectAllChk('" + idallchk + "','" + idchk + "')\"/>";

                    c.Header = keyname;
                    //c.Format = x => new HelperResult(writer => writer.Write("<input type=\"checkbox\" name=\"" + idchk + "\" value=\"" + x.Value.GetType().GetProperty(nomecolonna).GetValue(x, null).ToString() + "\" />"));
                    //var col = new GenericWebGridColumn(CreateColumn(format: (x) =>
                    //            _htmlHelper.CheckBox(idchk, new { @value = x.Value.GetType().GetProperty(nomecolonna).GetValue(x.Value, null).ToString(), 
                    //                                            @id = idchk + "_" + (x.Value.GetType().GetProperty(nomecolonna).GetValue(x.Value, null).ToString()).Trim() })));

                    var col = new GenericWebGridColumn(CreateColumn(format: (x) => MvcHtmlString.Create("<input type=\"checkbox\" name=\"" + idchk + "\" value=\"" +
                        x.Value.GetType().GetProperty(nomecolonna).GetValue(x.Value, null).ToString() + "\" id=\"" +
                        idchk + "_" + (x.Value.GetType().GetProperty(nomecolonna).GetValue(x.Value, null).ToString()).Trim() + "\"/>")));

                    c.Format = col.Format;
                    ht.Add(keyname, keyvalue);
                }
            }

            //IHtmlString htmlGrid = GetHtml(tableStyle: "genericwebgrid", htmlAttributes: _htmlAttributes, rowStyle: "r0", alternatingRowStyle: "r1", columns: Columns(_columnList.ToArray()));

            IHtmlString htmlGrid = GetHtml(tableStyle: "genericwebgrid", 
                                           htmlAttributes: _htmlAttributes, 
                                           rowStyle: "r0", 
                                           alternatingRowStyle: "r1", 
                                           columns: Columns(_columnList.ToArray()),
                                           caption: tableCaption.ToString());

            //return GetHtml(tableStyle: "genericwebgrid", htmlAttributes: _htmlAttributes, rowStyle: "r0", alternatingRowStyle: "r1", columns: Columns(_columnList.ToArray()));


            builder.Append(htmlGrid.ToString());
            foreach (DictionaryEntry d in ht)
            {
                builder.Replace(d.Key.ToString(), d.Value.ToString());
            }
            /* 31-08-2012 Commentato Old header DIV */
            //string result = "";
            //if ((TotalRowCount == 0))
            //{
            //    result = "<center><b>Nessun risultato trovato</b></center>";
            //}
            ...
            if ((!string.IsNullOrEmpty(AjaxUpdateContainerId)))
            {
                ...
                scriptBuilderHTML.Append("\n$(document).ready(function (){");
                //scriptBuilderHTML.AppendFormat("$({1}#{0}{1}).bind({1}ajaxSend{1},function () {{",
                //                                        _baseGrid.AjaxUpdateContainerId.ToString(),
                //                                        car.ToString());
                //scriptBuilderHTML.AppendFormat("$(this).html({1}<div valign='top'><center><img alt='Caricamento Dati' src='{0}' /> Caricamento dati ...</center></div>{1})", VirtualPathUtility.ToAbsolute("~/Images/round_preload.gif").ToString(), car.ToString());
                //scriptBuilderHTML.Append("});");
                scriptBuilderHTML.AppendFormat("$('#{0} table thead a,#{0} table tfoot a').live('click',\nfunction (event) {{", AjaxUpdateContainerId.ToString(), car.ToString());
                scriptBuilderHTML.AppendFormat("$('#{2}').html(\"<div valign='top'><center><img alt='Caricamento Dati' src='{0}' /> Caricamento dati ...</center></div>\");", VirtualPathUtility.ToAbsolute("~/Images/round_preload.gif").ToString(), car.ToString(), AjaxUpdateContainerId.ToString());
                scriptBuilderHTML.Append("\nreturn true;");
                scriptBuilderHTML.Append("});");
                scriptBuilderHTML.Append("})");
                scriptBuilder.InnerHtml = scriptBuilderHTML.ToString();
                builder.Append(scriptBuilder.ToString(TagRenderMode.Normal));
            }
        ...
        public string GetPageUrl(int pageIndex)
        {
            ...
            /* 16-10-2012 ausielra -- Gridname in querystring*/
            if (!String.IsNullOrEmpty(_querystringGridname))
            {
                queryString["gridsrc"] = _querystringGridname;
            }
            ...
            object htmlAttributes = null)
        {
            ...
            {
                var dictionary = System.Web.WebPages.Html.HtmlHelperGeneric.ObjectToDictionary(htmlAttributes);
            ...
            if (!String.IsNullOrEmpty(caption))
            {
                ...
                /* 31-08-2012 ausielra */
                /* for caption with button and total description */
                if (caption.ToLower().StartsWith("<table"))
                {
            ...
            /* 31-08-2012 ausielra */
            /* for row with description "NO DATA" */
            if (string.IsNullOrEmpty(tbody.InnerHtml))
            {
                tbody.InnerHtml = String.Format("<tr><td colspan=\"{0}\" align=\"center\"><b>Nessun risultato trovato</b></td></tr>",columns.Count().ToString());
            }
        ...
        internal static dynamic GetMember(GenericWebGridRow row, string name)
        {
            object result;
        ...
        public GenericWebGridColumn CreateColumn(string columnName = null, string header = null, System.Func<dynamic, MvcHtmlString> format = null, string style = null, bool canSort = true)
        {
            WebGridColumn baseColumn = Column(columnName, header, format, style, canSort);
            return new GenericWebGridColumn(baseColumn);
        }

        public GenericWebGridColumn CreateColumnCheckBox(string checkboxName, string checkboxValue, string style = "", bool selectall = true)
        {
            //string columnName = null, string header = null, System.Func<dynamic, MvcHtmlString> format = null, string style = null, bool canSort = true
            WebGridColumn baseColumn = Column(checkboxValue, "[]" + checkboxName, null, style, false);
            return new GenericWebGridColumnCheckBox(baseColumn, selectall);
        }

        /// <summary>
        /// Crea una colonna di tipo checkbox
        /// </summary>
        /// <param name="checkboxName">Valore assegnato al name della checkbox</param>
        /// <param name="checkboxValue">Valore assegnato al value della checkbox</param>
        /// <param name="style">[Opzionale] Nome dello stile che si vuole applicare alle checkbox</param>
        /// <returns></returns>
        //public GenericWebGridColumn CreateColumn(string checkboxName, string checkboxValue, string style = "")
        //{
        //    //string columnName = null, string header = null, System.Func<dynamic, MvcHtmlString> format = null, string style = null, bool canSort = true
        //    WebGridColumn baseColumn = Column(checkboxValue, "[]" + checkboxName, null, style, false);
        //    return new GenericWebGridColumn(baseColumn);
        ...
        public void AddColumns(List<GenericWebGridColumn> columnList)
        {
            _columnList = columnList;
        }

        public void AddImageLinkList(System.Func<object, MvcHtmlString> format = null)
        {
            _imageLinkList.Add(format.Invoke(null));
        }
    }
}